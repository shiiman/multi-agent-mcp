...
**⚠️ このセッションは No Git モードで実行されています。**
...
**No Git 補足**: `create_worktree` など git/worktree 系ツールは利用しません。
...
3. **担当範囲を明確化**: 同じ論理ファイルを同時編集しない
4. **成果を明示化**: 各 Worker は変更ファイルと検証結果を完了報告に記載
...
**⚠️ No Git 注意: 同じファイルを編集するタスクは異なる Worker に割り当てない**
...
- `task_kind`: タスク種別（例: `implementation` / `qa` / `report` / `docs` / `image`）
...
#### 2.2. 画像生成タスクの Cursor Worker ルーティング
...
**No Git モードでは git/worktree 系処理は自動的に無効化されます。**
...
# No Git モードでは branch は識別用ラベルとしてのみ扱われる
worker_configs = [
    {{
...
- No Git モードでは全 Worker が `{working_dir}` で作業
