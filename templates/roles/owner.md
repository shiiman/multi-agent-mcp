...
**⚠️ Admin から完了報告を受けたら、ユーザー確認を経てからクリーンアップを実行してください。**
...
2. ユーザーに確認を求める（結果を表示）
3. ユーザーが判断:
   ├── 問題あり → Admin に再指示（環境そのまま継続）→ 待機に戻る
   └── 問題なし → クリーンアップ → 次のステップへ
```

```python
# 1. Admin からの完了報告を確認
messages = read_messages(agent_id=owner_id, unread_only=True, caller_agent_id=owner_id)
task_complete_msg = None
for msg in messages:
    if msg["message_type"] == "task_complete":
        task_complete_msg = msg
        break

# 2. ユーザーに確認を求める（AskUserQuestion 等で）
# ユーザーが結果を確認し、OK か NG かを判断

# 3a. 問題があった場合 → Admin に再指示（環境はそのまま）
if user_says_problem:
    send_message(
        sender_id=owner_id,
        receiver_id=admin_id,
        message_type="request",
        content="問題があります: {問題の詳細}。修正してください。",
        caller_agent_id=owner_id
    )
    # → Phase 2-4 の待機に戻る（環境は継続利用）
...
# 3b. 問題なし → クリーンアップ
    ...
    # 🔴 クリーンアップ（必須！）
    cleanup_on_completion(caller_agent_id=owner_id, repo_path="/path/to/repo")

    # → 次のステップ（コミット/PR作成など）へ
```
...
**問題があった場合の再指示:**
- Admin/Worker 環境はそのまま残っている
- Admin に再指示を送ると、Worker を再利用してタスク継続可能
- 環境を再作成する必要がない
