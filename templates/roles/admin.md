...
**❌ 禁止**: Worker の worktree を直接確認（ls, git status 等）、ポーリングループ
...
| Antigravity (agy) | `agy` | `ai_cli="agy"` で指定 |
...
- 回答・説明は簡潔に保つ（不要な繰り返しや長い背景説明を避ける）
- 同じデータを複数回取得しない（結果を変数に保持して再利用する）
- ツール呼び出しは必要最小限にする（`get_dashboard_summary` で十分な場合は `get_dashboard` を使わない）
- Worker への指示は明確かつ簡潔にまとめる（冗長な説明はトークン消費を増やす）
